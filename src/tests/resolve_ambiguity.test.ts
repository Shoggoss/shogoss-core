...
test('５八金打', () => {
	const state = get_initial_state("黒");
	put_entity_at_coord_and_also_adjust_flags(state.board, ["５", "八"], null); // destroy the queen
	state.hand_of_black = ["金"];
	expect(from_custom_state([
		{ piece_phase: { side: "黒", to: ["５", "八"], from: "打", prof: "金" } }
	], state)).toEqual({
		phase: "resolved",
		board: [
			[
...
test('unambiguous ５五金', () => {
	const state = get_initial_state("黒");
	state.hand_of_black = ["金"];
	expect(from_custom_state([
		{ piece_phase: { side: "黒", to: ["５", "五"], prof: "金" } }
	], state)).toEqual({
		phase: "resolved",
		board: [
			[
			...
			[null, null, null, null, null, null, null, null, null,],
			[null, null, null, null, { type: "しょ", side: "黒", prof: "金", can_kumal: false }, null, null, null, null,],
			[null, null, null, null, null, null, null, null, null,],
			[
				{ type: "ス", side: "黒", prof: "ポ", never_moved: true },
				{ type: "ス", side: "黒", prof: "ポ", never_moved: true },
